...
async function calculateStorageUsedBytes(companyId) {
    ...
    while (stack.length) {
      ...
      for (const entry of entries) {
        ...
        try {
          ...
          logger.warn(`Cannot access file ${full}:`, error.message);
        }
      }
    }
  } catch (_) {
  ...
  try {
    // Оценка количества изображений из таблицы product_images
    const res = await db.query(
      `SELECT COALESCE(COUNT(*), 0)::int AS cnt
       FROM product_images pi
       JOIN products p ON pi.product_id = p.id
       WHERE p.company_id = $1 AND pi.is_active = true`,
      [companyId]
    );
...
// Инициализация Stripe, если есть ключ
  if (process.env.STRIPE_SECRET_KEY && billingService) {
...
router.get('/tariffs', async (req, res) => {
  try {
    const result = await db.query(`
      SELECT id, name, description, price, limits, features, is_active
      FROM tariffs
...
router.post('/create-payment-intent', authenticate, async (req, res) => {
  ...
  if (!billingService || !billingService.stripe) {
    ...
    const paymentIntent = await billingService.stripe.paymentIntents.create({
...
router.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  try {
    ...
    if (billingService.stripe && process.env.STRIPE_WEBHOOK_SECRET) {
      ...
      return res.status(503).json({ success: false, error: 'Billing webhook not configured' });
    }
