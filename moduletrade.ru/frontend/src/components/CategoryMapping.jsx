...
const CategoryMapping = () => {
    ...
    const loadSupplierCategories = async (supplierId) => {
        setLoading(true);
        try {
            const response = await api.get(`/suppliers/${supplierId}/categories`);
            setSupplierCategories(response.data.data);
        } catch (error) {
    ...
    const loadMappings = async (supplierId) => {
        try {
            const response = await api.get(`/mapping/categories?supplier_id=${supplierId}`);
            const mappingsObj = {};
