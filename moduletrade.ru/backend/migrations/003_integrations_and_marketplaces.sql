-- ================================================================
-- МИГРАЦИЯ 003: Интеграции и маркетплейсы (ИСПРАВЛЕНА И ОБНОВЛЕНА)
...
-- ИСПРАВЛЕНИЯ: Добавлены недостающие функции и улучшена обработка credentials
-- ================================================================
...
-- ================================================================
-- ДОПОЛНИТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ РАБОТЫ С CREDENTIALS
-- ================================================================

-- Функция для валидации и нормализации credentials
CREATE OR REPLACE FUNCTION validate_and_normalize_credentials(
    p_credentials JSONB,
    p_entity_type VARCHAR DEFAULT 'marketplace'
) RETURNS JSONB AS $$
DECLARE
    v_normalized_credentials JSONB;
    v_credentials_text TEXT;
BEGIN
    -- Если credentials уже JSONB, проверяем его валидность
    IF p_credentials IS NOT NULL AND jsonb_typeof(p_credentials) = 'object' THEN
        RETURN p_credentials;
    END IF;

    -- Если credentials - строка, пытаемся распарсить как JSON
    IF p_credentials IS NOT NULL AND jsonb_typeof(p_credentials) = 'string' THEN
        v_credentials_text := p_credentials::text;
        BEGIN
            -- Убираем лишние кавычки если они есть
            IF v_credentials_text LIKE '"%"' THEN
                v_credentials_text := substring(v_credentials_text from 2 for length(v_credentials_text) - 2);
            END IF;

            -- Пытаемся распарсить как JSON
            v_normalized_credentials := v_credentials_text::jsonb;
            RETURN v_normalized_credentials;
        EXCEPTION WHEN OTHERS THEN
            -- Если не удалось распарсить, создаем объект с raw_credentials
            RETURN jsonb_build_object('raw_credentials', v_credentials_text);
        END;
    END IF;

    -- Если credentials null или невалидный, возвращаем пустой объект
    RETURN '{}'::jsonb;
END;
$$ LANGUAGE plpgsql;

-- Функция для безопасного обновления credentials
CREATE OR REPLACE FUNCTION safe_update_credentials(
...
DECLARE
    ...
    v_result BOOLEAN;
BEGIN
    -- Нормализуем credentials
    v_normalized_credentials := validate_and_normalize_credentials(p_credentials, p_entity_type);

    -- Формируем SQL запрос для обновления
    v_sql := format('UPDATE %I SET credentials = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', p_table_name);

    -- Выполняем обновление
    EXECUTE v_sql USING v_normalized_credentials, p_record_id;
...
-- Функция для получения credentials с валидацией
CREATE OR REPLACE FUNCTION get_validated_credentials(
    p_table_name VARCHAR,
    p_record_id UUID
) RETURNS JSONB AS $$
DECLARE
    ...
    v_sql TEXT;
BEGIN
    -- Формируем SQL запрос для получения credentials
    v_sql := format('SELECT credentials FROM %I WHERE id = $1', p_table_name);

    -- Выполняем запрос
    EXECUTE v_sql INTO v_credentials USING p_record_id;

    -- Возвращаем валидированные credentials
    RETURN validate_and_normalize_credentials(v_credentials);
END;
$$ LANGUAGE plpgsql;

-- Функция для проверки структуры credentials
CREATE OR REPLACE FUNCTION validate_credentials_structure(
    p_credentials JSONB,
    p_required_fields TEXT[] DEFAULT ARRAY[]::TEXT[]
) RETURNS TABLE(
    is_valid BOOLEAN,
    missing_fields TEXT[],
...
BEGIN
    ...
    LOOP
        IF NOT (p_credentials ? v_field) THEN
            v_missing_fields := array_append(v_missing_fields, v_field);
            v_is_valid := FALSE;
        END IF;
    END LOOP;

    -- Проверяем, что credentials является объектом
    IF jsonb_typeof(p_credentials) != 'object' THEN
        v_validation_errors := array_append(v_validation_errors, 'Credentials must be a JSON object');
        v_is_valid := FALSE;
    END IF;
...
-- Функция для очистки устаревших credentials
...
DECLARE
    ...
    v_marketplaces_count INTEGER := 0;
BEGIN
    -- Очищаем credentials в таблице suppliers
    UPDATE suppliers
    ...
    -- Очищаем credentials в таблице marketplaces
    UPDATE marketplaces
    SET credentials = '{}'::jsonb,
        updated_at = CURRENT_TIMESTAMP
    WHERE updated_at < CURRENT_TIMESTAMP - INTERVAL '1 day' * p_days_old
    ...
    GET DIAGNOSTICS v_marketplaces_count = ROW_COUNT;

    v_cleaned_count := v_suppliers_count + v_marketplaces_count;
