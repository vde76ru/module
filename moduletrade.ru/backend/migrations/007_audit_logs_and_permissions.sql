-- ================================================================
-- МИГРАЦИЯ 007: Аудит и разрешения (ОБНОВЛЕНА И ИСПРАВЛЕНА)
...
-- ИСПРАВЛЕНИЯ: Добавлены недостающие функции и улучшена обработка сессий
-- ================================================================
...
-- ================================================================
-- ДОПОЛНИТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ РАБОТЫ С СЕССИЯМИ
-- ================================================================

-- Функция для создания или обновления сессии пользователя
...
-- Функция для деактивации сессии пользователя
CREATE OR REPLACE FUNCTION deactivate_audit_session(
    p_session_id VARCHAR(255)
) RETURNS BOOLEAN AS $$
DECLARE
...
BEGIN
    ...
    WHERE is_active = TRUE
      AND last_activity < CURRENT_TIMESTAMP - INTERVAL '1 hour' * p_inactive_hours;
...
CREATE OR REPLACE FUNCTION validate_audit_session(
    p_session_id VARCHAR(255)
) RETURNS TABLE(
    is_valid BOOLEAN,
    user_id UUID,
...
BEGIN
    RETURN QUERY
    SELECT
        a.is_active AND a.last_activity > CURRENT_TIMESTAMP - INTERVAL '24 hours' as is_valid,
        a.user_id,
        a.company_id,
        a.last_activity
    FROM audit_sessions a
