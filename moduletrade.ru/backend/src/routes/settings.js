 ...
 * Алиас для получения настроек (для соответствия фронтенду)
 */
 ...
 * Алиас для обновления настроек (для соответствия фронтенду)
 */
router.put('/profile', authenticate, checkRole(['admin', 'manager']), async (req, res) => {
  try {
    const companyId = req.user.companyId;
    const updates = req.body;
