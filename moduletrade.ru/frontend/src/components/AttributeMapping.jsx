...
const AttributeMapping = () => {
    ...
    // Используем единый axios instance
    const api = axios;
            ...
            render: (_, record) => {
                ...
                return (
                    <Space direction="vertical" style={{ width: '100%' }}>
                        {Array.isArray(attributeMappings) ? attributeMappings.map(mapping => (
                            <Tag
                                key={mapping.id}
                                closable
                                    ...
                                    {({ getFieldValue }) =>
                                        getFieldValue(['unit_conversion', 'enabled']) && (
                                            <>
                                                ...
                                                >
                                                    <Select>
                                                        {Array.isArray(unitTypes[attribute.unit_type]?.units) ? unitTypes[attribute.unit_type].units.map(unit => (
    ...
    const ValueMappingEditor = ({ systemValues, value, onChange }) => {
        ...
        return (
            <div>
                <Space direction="vertical" style={{ width: '100%' }}>
                    {Array.isArray(Object.entries(mappings)) ? Object.entries(mappings).map(([supplierValue, systemValue]) => (
                        <Input.Group key={supplierValue} compact>
                            ...
                            >
                                {Array.isArray(systemValues) ? systemValues.map(v => (
