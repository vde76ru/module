...
class ProductImportJob {
  ...
  /**
   * Импорт всех брендов, привязанных к поставщику в компании
   */
  static async importSupplierForCompany(companyId, supplierId, brandIds = null) {
    try {
      const mappings = await this.getBrandMappingsForSupplier(companyId, supplierId);
      const effectiveBrandIds = Array.isArray(brandIds) && brandIds.length > 0
        ? mappings.filter(m => brandIds.includes(m.brand_id)).map(m => m.brand_id)
        : mappings.map(m => m.brand_id);

      for (const brandId of effectiveBrandIds) {
        await this.importBrandFromSupplier(companyId, supplierId, brandId, mappings[0]?.supplier_name || 'Supplier', mappings.find(m => m.brand_id === brandId)?.brand_name || 'Brand');
      }
    } catch (error) {
      logger.error(`Error importing supplier ${supplierId} for company ${companyId}:`, error);
      throw error;
    }
  }

  /**
   * Получить маппинги брендов по поставщику
   */
  static async getBrandMappingsForSupplier(companyId, supplierId) {
    const result = await db.query(`
      SELECT
        bsm.brand_id,
