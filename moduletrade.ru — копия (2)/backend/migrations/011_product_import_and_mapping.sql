...
ALTER TABLE brand_supplier_mappings
    ADD CONSTRAINT ck_brand_supplier_mappings_public_id CHECK (public_id IS NULL OR public_id >= 100000);
CREATE UNIQUE INDEX IF NOT EXISTS uq_brand_supplier_mappings_public_id ON brand_supplier_mappings (public_id) WHERE public_id IS NOT NULL;

CREATE TRIGGER assign_brand_supplier_mappings_public_id
    BEFORE INSERT ON brand_supplier_mappings
    FOR EACH ROW
...
ALTER TABLE name_processing_rules
    ADD CONSTRAINT ck_name_processing_rules_public_id CHECK (public_id IS NULL OR public_id >= 100000);
CREATE UNIQUE INDEX IF NOT EXISTS uq_name_processing_rules_company_public_id ON name_processing_rules (company_id, public_id) WHERE public_id IS NOT NULL;

CREATE TRIGGER assign_name_processing_rules_public_id
    BEFORE INSERT ON name_processing_rules
    FOR EACH ROW
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_attribute_mappings_company_public_id ON attribute_mappings (company_id, public_id) WHERE public_id IS NOT NULL;
...
ALTER TABLE category_mappings
    ADD CONSTRAINT ck_category_mappings_public_id CHECK (public_id IS NULL OR public_id >= 100000);
CREATE UNIQUE INDEX IF NOT EXISTS uq_category_mappings_company_public_id ON category_mappings (company_id, public_id) WHERE public_id IS NOT NULL;

CREATE TRIGGER assign_category_mappings_public_id
    BEFORE INSERT ON category_mappings
    FOR EACH ROW
    EXECUTE FUNCTION assign_public_id_with_company();
...
ALTER TABLE external_categories
    ADD CONSTRAINT ck_external_categories_public_id CHECK (public_id IS NULL OR public_id >= 100000);
CREATE UNIQUE INDEX IF NOT EXISTS uq_external_categories_public_id ON external_categories (public_id) WHERE public_id IS NOT NULL;

CREATE TRIGGER assign_external_categories_public_id
    BEFORE INSERT ON external_categories
    FOR EACH ROW
    EXECUTE FUNCTION assign_public_id_global();
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_attribute_values_public_id ON attribute_values (public_id) WHERE public_id IS NOT NULL;

CREATE TRIGGER assign_attribute_values_public_id
...
ALTER TABLE order_processing_rules
    ADD CONSTRAINT ck_order_processing_rules_public_id CHECK (public_id IS NULL OR public_id >= 100000);
CREATE UNIQUE INDEX IF NOT EXISTS uq_order_processing_rules_company_public_id ON order_processing_rules (company_id, public_id) WHERE public_id IS NOT NULL;

CREATE TRIGGER assign_order_processing_rules_public_id
    BEFORE INSERT ON order_processing_rules
    FOR EACH ROW
    EXECUTE FUNCTION assign_public_id_with_company();
...
ALTER TABLE pricing_settings
    ADD CONSTRAINT ck_pricing_settings_public_id CHECK (public_id IS NULL OR public_id >= 100000);
CREATE UNIQUE INDEX IF NOT EXISTS uq_pricing_settings_company_public_id ON pricing_settings (company_id, public_id) WHERE public_id IS NOT NULL;

CREATE TRIGGER assign_pricing_settings_public_id
