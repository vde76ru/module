...
class AuditService {
  ...
  async logAction(actionData) {
    try {
      ...
      const actionTypeResult = await db.query(
        'SELECT id FROM audit_action_types WHERE name = $1',
        [action]
      );
      ...
      if (actionTypeResult.rows.length === 0) {
        ...
        const newActionTypeResult = await db.query(`
          INSERT INTO audit_action_types (name, display_name, category, description)
          VALUES ($1, $2, 'custom', $3)
          RETURNING id
        `, [action, action.replace(/[._]/g, ' '), `Автоматически созданное действие: ${action}`]);
      ...
      const auditLogResult = await db.query(`
        INSERT INTO audit_logs (
          company_id, user_id, session_id, action_type_id, action,
          ...
          success, error_message, metadata, tags, created_at
        ) VALUES (
    ...
    try {
      ...
      await db.query(`
        INSERT INTO audit_sessions (
          id, company_id, user_id, ip_address, user_agent,
  ...
  async updateSessionActivity(sessionId) {
    try {
      await db.query(
        'UPDATE audit_sessions SET last_activity = NOW() WHERE id = $1',
        [sessionId]
      );
    } catch (error) {
  ...
  async endSession(sessionId) {
    try {
      ...
      const sessionResult = await db.query(
        'SELECT company_id, user_id FROM audit_sessions WHERE id = $1',
        [sessionId]
      );
  ...
  async getAuditLogs(companyId, filters = {}) {
    try {
      let query = `
        SELECT 
          al.id, al.created_at, al.action, al.entity_type, al.entity_id, 
          al.entity_name, al.description, al.success, al.error_message,
          al.ip_address, al.tags,
          aat.display_name as action_display, aat.category as action_category,
          aat.severity as action_severity,
          u.name as user_name, u.email as user_email,
          al.old_values, al.new_values, al.changes, al.metadata
        FROM audit_logs al
      ...
      if (filters.search) {
        query += ` AND (
          ...
          aat.display_name ILIKE $${paramIndex}
        )`;
  ...
  async getEntityAuditTrail(companyId, entityType, entityId, options = {}) {
    try {
      ...
      const result = await db.query(`
        SELECT 
          al.id, al.created_at, al.action, al.description,
          al.old_values, al.new_values, al.changes,
          aat.display_name as action_display, aat.severity,
          u.name as user_name, u.email as user_email
        FROM audit_logs al
        LEFT JOIN users u ON al.user_id = u.id
        JOIN audit_action_types aat ON al.action_type_id = aat.id
