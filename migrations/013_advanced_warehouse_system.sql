...
BEGIN
    ...
    SELECT p.*, b.wholesale_markup, b.retail_markup, b.website_markup, b.marketplace_markup
    INTO v_product
    FROM products p
    ...
    -- Проверяем МРЦ/РРЦ
    IF v_product.mrp_price IS NOT NULL AND v_final_price < v_product.mrp_price THEN
        v_final_price := v_product.mrp_price;
    END IF;

    IF v_product.rrp_price IS NOT NULL AND v_final_price > v_product.rrp_price THEN
        v_final_price := v_product.rrp_price;
    END IF;
