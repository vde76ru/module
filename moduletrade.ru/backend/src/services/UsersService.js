...
// Простая проверка надежности пароля, согласованная с дефолтными настройками в миграции 008
// - Минимум 8 символов
...
class UsersService {
  ...
  async createUser(companyId, userData) {
    const { email, password, name, role, role_id: explicitRoleId } = userData;
    ...
    // Определяем роль пользователя (role_id обязателен по схеме)
    let resolvedRoleId = explicitRoleId || null;
    if (!resolvedRoleId) {
      if (role) {
        const byName = await db.query(`SELECT id FROM roles WHERE name = $1 OR code = $1 LIMIT 1`, [role]);
        if (byName.rows.length > 0) {
          resolvedRoleId = byName.rows[0].id;
        }
      }
      if (!resolvedRoleId) {
        const fallback = await db.query(`SELECT id FROM roles WHERE is_system = true AND (code = 'manager' OR name = 'Менеджер') LIMIT 1`);
        if (fallback.rows.length > 0) {
          resolvedRoleId = fallback.rows[0].id;
        }
      }
    }

    if (!resolvedRoleId) {
      throw new Error('Unable to resolve role_id for user');
    }

    // Создаем пользователя (роль в текстовом поле сохраняем для совместимости)
    const result = await db.query(
      `INSERT INTO users (id, company_id, email, password_hash, name, role_id, role, is_active)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING id, email, name, role_id, is_active, created_at`,
      [uuidv4(), companyId, email, passwordHash, name, resolvedRoleId, role || null, true]
    );
    ...
    const accessToken = jwt.sign(
      {
        ...
        email: user.email,
        role: user.role_name || user.role || 'Пользователь'
      },
  ...
  async refreshToken(refreshToken) {
    try {
      ...
      const result = await db.query(
        `SELECT u.id, u.email, u.company_id, r.name as role_name
         FROM users u
         LEFT JOIN roles r ON u.role_id = r.id
         WHERE u.id = $1 AND u.is_active = true AND u.company_id = $2`,
      ...
      const accessToken = jwt.sign(
        {
          ...
          email: user.email,
          role: user.role_name || 'Пользователь'
        },
        process.env.JWT_SECRET,
        { expiresIn: '24h' }
      );
  ...
  async getUsers(companyId, filters = {}) {
    ...
    if (filters.role) {
      ...
      query += ` AND (r.name = $${params.length} OR r.code = $${params.length})`;
    }
  ...
  async updateUser(companyId, userId, updates) {
    ...
    // Обновление роли через role_id или role/code
    if (updates.role_id) {
      params.push(updates.role_id);
      setClause.push(`role_id = $${paramIndex++}`);
    } else if (updates.role) {
      const roleRes = await db.query(`SELECT id FROM roles WHERE name = $1 OR code = $1 LIMIT 1`, [updates.role]);
