  ...
  try {
    ...
    const result = await db.query(`
      SELECT
        ...
        u.company_id, u.is_active, u.last_login,
        u.role_id, r.name as role_name, r.name as role_display_name,
        c.name as company_name, c.subscription_status as company_status, c.settings as company_settings
      FROM users u
    ...
    req.user = {
      ...
      name: user.name,
      phone: user.phone,
      role: user.role_name,  // Используем role_name как основное поле роли
      roleId: user.role_id,
...
const checkPermission = (requiredPermission) => {
  return async (req, res, next) => {
    ...
    // Владельцы и Администраторы имеют все права по определению
    if (roleName === 'Владелец' || roleName === 'Администратор') {
      return next();
    }

    // Если нет role_id, но есть поле role - используем его для совместимости
    ...
    try {
      // Проверяем разрешения через JSON поле permissions в таблице roles
      ...
      // Формат: "products.create" -> проверяем permissions.products содержит "create"
