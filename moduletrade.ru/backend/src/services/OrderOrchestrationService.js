...
class OrderOrchestrationService {
  ...
  async getOrderStatus(orderId, companyId) {
    try {
      const result = await db.query(`
        SELECT
          o.*,
          ms.name as marketplace_name,
          ms.code as marketplace_code,
        ...
        LEFT JOIN marketplaces ms ON o.marketplace_id = ms.id
        LEFT JOIN order_items oi ON o.id = oi.order_id
        WHERE o.id = $1 AND o.company_id = $2
        GROUP BY o.id, ms.name, ms.code
      `, [orderId, companyId]);
  ...
  async getOrders(companyId, filters = {}) {
    try {
      ...
      query += ` GROUP BY o.id, ms.name, ms.code`;
