...
class ProductModel {
  ...
  static async create(companyId, productData) {
    const {
      ...
      height, dimensions, is_active = true
    } = productData;
    ...
    const query = `
      INSERT INTO products (
        ...
        min_order_quantity, weight, length, width, height, dimensions, is_active
...
class SyncLogModel {
  static async create(companyId, syncData) {
    const {
      sync_type, status = 'pending', details = {},
      started_at = new Date()
    } = syncData;
    ...
    const result = await db.query(query, [companyId, sync_type, status, details, started_at]);
    return result.rows[0];
  }
...
class ApiLogModel {
  static async create(logData) {
    const {
      ...
      request_body, response_code, response_time_ms, ip_address, user_agent
    } = logData;
