...
router.post('/transfer', authenticate, checkPermission('warehouses.transfer'), async (req, res) => {
  ...
  try {
    ...
    // Записываем движение товара (расход с исходного склада)
    await client.query(`
      INSERT INTO warehouse_movements (
        warehouse_id, product_id, movement_type, quantity,
        reference_type, reference_id, description, created_by
    ...
    `, [
      ...
      'transfer',
      null,
      `Transfer to ${validation.to_warehouse_name}: ${reason}`,
      req.user.id
    ]);

    // Записываем движение товара (приход на целевой склад)
    await client.query(`
      INSERT INTO warehouse_movements (
        warehouse_id, product_id, movement_type, quantity,
        reference_type, reference_id, description, created_by
    ...
    `, [
      ...
      'transfer',
      null,
      `Transfer from ${validation.from_warehouse_name}: ${reason}`,
      req.user.id
    ]);
