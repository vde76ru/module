-- ================================================================
-- МИГРАЦИЯ 004: PIM система - товары и атрибуты (ОБНОВЛЕНА И ИСПРАВЛЕНА)
...
-- ИСПРАВЛЕНИЯ: Добавлены недостающие функции и оптимизированы запросы
-- ================================================================
...
-- Функция для получения количества изображений товара
CREATE OR REPLACE FUNCTION get_product_images_count(p_product_id UUID)
RETURNS INTEGER AS $$
DECLARE
    v_count INTEGER;
BEGIN
...
-- Функция для получения основного изображения товара
CREATE OR REPLACE FUNCTION get_product_main_image(p_product_id UUID)
RETURNS TABLE(
    image_url TEXT,
    url TEXT,
    alt_text TEXT,
...
CREATE OR REPLACE FUNCTION get_product_all_images(p_product_id UUID)
RETURNS TABLE(
    image_url TEXT,
    url TEXT,
    alt_text TEXT,
...
-- Функция для подсчета общего количества изображений в компании
CREATE OR REPLACE FUNCTION get_company_images_count(p_company_id UUID)
RETURNS INTEGER AS $$
DECLARE
    v_count INTEGER;
BEGIN
...
-- Функция для обновления статистики изображений товара
CREATE OR REPLACE FUNCTION update_product_images_stats(p_product_id UUID)
RETURNS VOID AS $$
DECLARE
...
BEGIN
    ...
    SELECT COALESCE(image_url, url) INTO v_main_image_url
    FROM product_images
    ...
    -- Обновляем статистику в таблице products (если нужно)
    -- Здесь можно добавить обновление дополнительных полей
END;
$$ LANGUAGE plpgsql;
