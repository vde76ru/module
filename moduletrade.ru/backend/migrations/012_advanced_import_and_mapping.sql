...
ALTER TABLE supplier_product_mappings
    ADD CONSTRAINT ck_supplier_product_mappings_public_id CHECK (public_id IS NULL OR public_id >= 100000);
CREATE UNIQUE INDEX IF NOT EXISTS uq_supplier_product_mappings_company_public_id ON supplier_product_mappings (company_id, public_id) WHERE public_id IS NOT NULL;

CREATE TRIGGER assign_supplier_product_mappings_public_id
    BEFORE INSERT ON supplier_product_mappings
    FOR EACH ROW
...
ALTER TABLE product_matching_rules
    ADD CONSTRAINT ck_product_matching_rules_public_id CHECK (public_id IS NULL OR public_id >= 100000);
CREATE UNIQUE INDEX IF NOT EXISTS uq_product_matching_rules_company_public_id ON product_matching_rules (company_id, public_id) WHERE public_id IS NOT NULL;

CREATE TRIGGER assign_product_matching_rules_public_id
    BEFORE INSERT ON product_matching_rules
    FOR EACH ROW
    EXECUTE FUNCTION assign_public_id_with_company();
...
ALTER TABLE import_sessions
    ADD CONSTRAINT ck_import_sessions_public_id CHECK (public_id IS NULL OR public_id >= 100000);
CREATE UNIQUE INDEX IF NOT EXISTS uq_import_sessions_company_public_id ON import_sessions (company_id, public_id) WHERE public_id IS NOT NULL;

CREATE TRIGGER assign_import_sessions_public_id
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_import_logs_public_id ON import_logs (public_id) WHERE public_id IS NOT NULL;
