...
const ProcurementCenter = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (activeTab === 0) {
        const draftsRes = await axios.get('/api/procurement/supplier-orders', { params: { status: 'draft' } });
        const orders = draftsRes.data?.orders || draftsRes.data || [];
        setDraftOrders(orders);
      } else {
        const historyRes = await axios.get('/api/procurement/supplier-orders', {
          params: {
  ...
  const handleSendOrders = async () => {
    try {
      ...
      await axios.post('/api/procurement/send-orders', { orderIds: ordersToSend });
            ...
            {draftOrders.map((order) => (
              <React.Fragment key={order.id}>
                <TableRow>
                  ...
                  <TableCell>{getSupplierName(order.main_supplier_id)}</TableCell>
