...
ALTER TABLE cache_keys
    ADD CONSTRAINT ck_cache_keys_public_id CHECK (public_id IS NULL OR public_id >= 100000);
CREATE UNIQUE INDEX IF NOT EXISTS uq_cache_keys_public_id ON cache_keys (public_id) WHERE public_id IS NOT NULL;

CREATE TRIGGER assign_cache_keys_public_id
    BEFORE INSERT ON cache_keys
    FOR EACH ROW
...
ALTER TABLE cache_invalidation_rules
    ADD CONSTRAINT ck_cache_invalidation_rules_public_id CHECK (public_id IS NULL OR public_id >= 100000);
CREATE UNIQUE INDEX IF NOT EXISTS uq_cache_invalidation_rules_public_id ON cache_invalidation_rules (public_id) WHERE public_id IS NOT NULL;

CREATE TRIGGER assign_cache_invalidation_rules_public_id
    BEFORE INSERT ON cache_invalidation_rules
    FOR EACH ROW
    EXECUTE FUNCTION assign_public_id_global();
...
ALTER TABLE performance_metrics
    ADD CONSTRAINT ck_performance_metrics_public_id CHECK (public_id IS NULL OR public_id >= 100000);
CREATE UNIQUE INDEX IF NOT EXISTS uq_performance_metrics_public_id ON performance_metrics (public_id) WHERE public_id IS NOT NULL;

CREATE TRIGGER assign_performance_metrics_public_id
...
ALTER TABLE sync_schedules
    ADD CONSTRAINT ck_sync_schedules_public_id CHECK (public_id IS NULL OR public_id >= 100000);
CREATE UNIQUE INDEX IF NOT EXISTS uq_sync_schedules_company_public_id ON sync_schedules (company_id, public_id) WHERE public_id IS NOT NULL;

CREATE TRIGGER assign_sync_schedules_public_id
...
ALTER TABLE data_change_logs
    ADD CONSTRAINT ck_data_change_logs_public_id CHECK (public_id IS NULL OR public_id >= 100000);
CREATE UNIQUE INDEX IF NOT EXISTS uq_data_change_logs_company_public_id
  ON data_change_logs (company_id, public_id)
  WHERE public_id IS NOT NULL AND company_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS uq_data_change_logs_public_id_global
  ON data_change_logs (public_id)
  WHERE public_id IS NOT NULL AND company_id IS NULL;

CREATE TRIGGER assign_data_change_logs_public_id
    BEFORE INSERT ON data_change_logs
    FOR EACH ROW
    EXECUTE FUNCTION assign_public_id_with_company();
...
BEGIN
    ...
    PERFORM invalidate_cache('products', 'product', COALESCE(NEW.id, OLD.id));
...
BEGIN
    ...
    PERFORM invalidate_cache('prices', 'product', COALESCE(NEW.product_id, OLD.product_id));
...
BEGIN
    ...
    PERFORM invalidate_cache('stocks', 'product', COALESCE(NEW.product_id, OLD.product_id));
    PERFORM invalidate_cache('stocks', 'warehouse', COALESCE(NEW.warehouse_id, OLD.warehouse_id));
...
-- Создаем базовые расписания синхронизации (ИСПРАВЛЕНО: убраны ссылки на несуществующую компанию)
-- Примечание: Расписания будут создаваться для конкретных компаний при их регистрации
