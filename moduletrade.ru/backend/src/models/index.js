...
class ProductModel {
  static async findById(companyId, productId) {
    ...
    // Подтягиваем изображения из product_images
  ...
  static async findAll(companyId, filters = {}) {
    ...
    const query = `
      ...
      LEFT JOIN (
        SELECT
          wpl.product_id,
          SUM(COALESCE(wpl.available_quantity, 0))::numeric as total_stock,
          MIN(COALESCE(wpl.price, wpl.retail_price))::numeric as min_price
