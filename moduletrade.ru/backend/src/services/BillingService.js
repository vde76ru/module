...
class BillingService {
  ...
  async initializeTariffs() {
    const result = await db.query('SELECT * FROM tariffs WHERE is_active = true');
  ...
  // Создание подписки
  async createSubscription(companyId, tariffCode, paymentMethodId) {
  ...
  // Проверка лимитов
  async checkLimit(companyId, limitType, currentValue = 0) {
  ...
  async chargeAPIUsage(companyId, apiCalls) {
