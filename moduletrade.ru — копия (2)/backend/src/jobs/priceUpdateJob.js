...
class PriceUpdateJob {
  ...
  /**
   * Обновление цен для конкретной компании
   */
  static async updatePricesForCompany(companyId) {
    try {
      // Получаем компанию
      ...
      if (companyResult.rows.length === 0) {
        logger.warn(`Company not found or inactive for price update: ${companyId}`);
        return;
      }
      ...
      logger.error(`Error updating prices for company ${companyId}:`, error);
      throw error;
    }
  }
