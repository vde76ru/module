...
class WarehouseService {
  ...
  static async createWarehouse(companyId, warehouseData) {
    const query = `
      INSERT INTO warehouses (
        company_id, name, type, city, address,
    ...
    const result = await db.query(query, [
      companyId, warehouseData.name, warehouseData.type, warehouseData.city || null,
  ...
  static async getWarehouses(companyId, filters = {}) {
    ...
    const query = `
      SELECT w.*,
             COUNT(DISTINCT wmi.marketplace_id) as connected_marketplaces,
             COUNT(DISTINCT ws.product_id) as total_products
      FROM warehouses w
      LEFT JOIN warehouse_marketplace_integrations wmi ON w.id = wmi.warehouse_id AND wmi.is_active = true
  ...
  static async connectMarketplace(warehouseId, marketplaceId, settings = {}) {
    const query = `
      INSERT INTO warehouse_marketplace_integrations (
  ...
  static async disconnectMarketplace(warehouseId, marketplaceId) {
    const query = `
      UPDATE warehouse_marketplace_integrations
    ...
    try {
      ...
      const stockQuery = `
        INSERT INTO warehouse_product_links (
          warehouse_id, product_id, quantity, purchase_price,
      ...
      await client.query(stockQuery, [
        ...
        priceData.purchase_price || priceData.price || 0
      ]);
  ...
  static async getMarketplaceSettings(warehouseId) {
    const query = `
      ...
      FROM marketplaces m
      JOIN warehouse_marketplace_integrations wmc ON m.id = wmc.marketplace_id
  ...
  static async getHighProfitProducts(warehouseId, limit = 10) {
    const query = `
      SELECT ws.*, p.name as product_name, p.sku,
              COALESCE(SUM(oi.quantity * (oi.unit_price - COALESCE(ws.purchase_price, 0))), 0) as total_profit,
              COALESCE(AVG(oi.unit_price - COALESCE(ws.purchase_price, 0)), 0) as avg_profit_per_unit
      FROM warehouse_product_links ws
      JOIN products p ON ws.product_id = p.id
      LEFT JOIN order_items oi ON p.id = oi.product_id
      LEFT JOIN orders o ON oi.order_id = o.id
