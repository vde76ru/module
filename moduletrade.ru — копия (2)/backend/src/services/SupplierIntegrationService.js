  ...
  static async updateProductSupplierLink(client, companyId, productId, supplierId, productData) {
    ...
    await client.query(
      ...
      DO UPDATE SET
        ...
        partner_stock_quantity = EXCLUDED.partner_stock_quantity,
        estimated_arrival_date = EXCLUDED.estimated_arrival_date,
        price_personal = EXCLUDED.price_personal,
        price_personal_vat = EXCLUDED.price_personal_vat,
        price_retail = EXCLUDED.price_retail,
        price_retail_vat = EXCLUDED.price_retail_vat,
      ...
      [
        ...
        price?.personal ?? price?.retail ?? 0,
        stock?.quantity ?? 0,
        stock?.partnerStock?.quantity ?? 0,
        stock?.partnerStock?.estimatedArrivalDate ? new Date(stock.partnerStock.estimatedArrivalDate) : null,
        price?.personal,
        price?.personalWithVAT,
        price?.retail,
        price?.retailWithVAT,
        price?.mrc,
        JSON.stringify({
        ...
        (Number(stock?.quantity || 0) > 0) || (Number(stock?.partnerStock?.quantity || 0) > 0)
      ]
    );
  }
