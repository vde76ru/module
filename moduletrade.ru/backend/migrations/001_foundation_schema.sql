...
-- Версия: 1.0
-- Дата: 2025-08-04
-- Описание: Фундаментальная схема базы данных
-- ========================================
...
-- ИСПРАВЛЕННАЯ функция для логирования изменений важных сущностей
...
DECLARE
    ...
    record_json JSONB;
BEGIN
    -- Определяем действие и ID записи
    ...
    -- ИСПРАВЛЕНО: Определяем company_id в зависимости от таблицы
    IF table_name_val = 'companies' THEN
        -- Для таблицы companies, company_id = id самой записи
        company_id_val := record_id_val;
    ELSE
        -- Для других таблиц пытаемся взять поле company_id из JSON
        BEGIN
    ...
    -- Получаем ID пользователя из контекста сессии
    BEGIN
...
EXCEPTION WHEN OTHERS THEN
    -- В случае ошибки не прерываем основную операцию, просто логируем
...
COMMENT ON FUNCTION log_entity_changes() IS 'Исправленная функция логирования изменений с корректной обработкой company_id для разных таблиц';
...
-- Триггеры для логирования важных изменений
DO $$
BEGIN
...
-- Комментарии для важных полей
COMMENT ON COLUMN companies.subscription_status IS 'Статус подписки: inactive, trial, active, past_due, canceled, unpaid';
COMMENT ON COLUMN companies.settings IS 'Настройки компании в формате JSON';
COMMENT ON COLUMN users.role IS 'Старое поле роли для обратной совместимости';
COMMENT ON COLUMN users.role_id IS 'Новое поле роли для RBAC системы';
COMMENT ON COLUMN logs.meta IS 'Дополнительные метаданные лога в JSON формате';
...
BEGIN
    ...
    RAISE NOTICE 'Fixed log_entity_changes function for proper company_id handling';
END $$;