...
class SyncService {
  constructor() {
    ...
    this.marketplaceConfigs = {
      ...
      // Яндекс.Маркет: поддерживаем оба ключа для совместимости ('yandex' и 'yandex_market')
      yandex: {
  ...
  async fullSync(companyId, marketplaceId) {
    ...
    try {
      ...
      // Обновляем время последней синхронизации интеграции
      await client.query(`
        ...
        SET last_sync_at = NOW(),
            ...
            END,
            updated_at = NOW()
        WHERE company_id = $1 AND marketplace_id = $2
      `, [
        companyId,
        marketplaceId,
        syncResults.products.errors + syncResults.orders.errors + syncResults.stocks.errors
  ...
  async fetchSupplierStockData(supplier, apiConfig) {
    // Подключаемся к поставщику через фабрику адаптеров
    ...
    // Ожидается, что адаптер реализует метод getStockLevels()
    if (typeof adapter.getStockLevels !== 'function') {
      throw new Error(`Supplier adapter ${supplier.api_type} does not implement getStockLevels()`);
    }
    const stockLevels = await adapter.getStockLevels();
    // Нормализуем к единому формату
    return stockLevels.map(item => ({
      vendor_sku: item.vendor_sku || item.sku || item.article || item.code,
      quantity: Number(item.quantity ?? item.available ?? 0),
  ...
  async fetchMarketplaceProducts(marketplace) {
    ...
    if (typeof adapter.getProducts !== 'function') {
      throw new Error(`Marketplace adapter ${marketplace.type} does not implement getProducts()`);
    }
    const { products } = await adapter.getProducts({ limit: 100 });
    return products.map(p => ({
      external_id: p.offer_id || p.id || p.external_id,
      name: p.name,
  ...
  async fetchMarketplaceOrders(marketplace) {
    ...
    if (typeof adapter.getOrders !== 'function') {
      throw new Error(`Marketplace adapter ${marketplace.type} does not implement getOrders()`);
    }
    const { orders } = await adapter.getOrders({ limit: 100 });
    return orders.map(o => ({
      marketplace_order_id: o.posting_number || o.id || o.order_id,
      order_number: o.posting_number || o.number || o.order_number,
      customer_name: o.customer?.name || null,
      customer_email: o.customer?.email || null,
      status: o.status || 'new',
      payment_status: o.payment_status || null,
      total_amount: o.total_amount || o.analytics_data?.revenue || 0,
      currency: o.currency || 'RUB',
      order_date: o.in_process_at || o.created_at || new Date(),
  ...
  async updateMarketplaceStocks(marketplace, stockUpdates) {
    ...
    const factory = new MarketplaceFactory();
    const adapter = factory.createAdapter(marketplace.type, credentials || {});
    if (typeof adapter.updateStock !== 'function') {
      throw new Error(`Marketplace adapter ${marketplace.type} does not implement updateStock()`);
    }
    for (const upd of stockUpdates) {
      await adapter.updateStock(upd.productId || upd.offer_id || upd.sku, upd.quantity);
    }
    return true;
  }
  ...
  async getSyncLogs(companyId, filters = {}) {
    try {
      let query = `
        SELECT sl.*, m.name as marketplace_name, m.type as marketplace_type
        FROM sync_logs sl
