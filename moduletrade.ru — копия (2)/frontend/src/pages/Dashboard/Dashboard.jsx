...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setRecentOrders(ordersRes.status === 'fulfilled' ? (ordersRes.value?.orders || ordersRes.value?.data || []) : []);
      setLowStockProducts(stockRes.status === 'fulfilled' ? (stockRes.value?.products || stockRes.value?.data || []) : []);
      setSyncStatus(syncRes.status === 'fulfilled' ? (syncRes.value || []) : []);
    } catch (error) {
      console.error('Dashboard data fetch error:', error);
      // Не показываем демо-данные — оставляем пусто
      setStatistics({});
      setRecentOrders([]);
      setLowStockProducts([]);
    } finally {
