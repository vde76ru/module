...
class PIMService {
  ...
  async getAllProducts(companyId, filters = {}) {
    try {
      ...
      if (productIds.length > 0) {
        // Используем поле products.images (JSONB) для главного изображения
        const imgRes = await db.query(
          `SELECT id as product_id, images FROM products WHERE id = ANY($1)`,
          [productIds]
        );
        ...
        for (const row of imgRes.rows) {
          ...
          // выбираем главное или первое
          const main = imagesArray.find(i => i.is_main) || imagesArray[0];
          if (!main) continue;
          const url = main.image_url || main.url;
          if (!url) continue;
          const token = cryptoUtils.encrypt(JSON.stringify({ u: url, p: row.product_id }));
          imagesMap.set(row.product_id, {
            image_id: main.id || null,
            image_url: url,
  ...
  async getProductById(companyId, productId) {
    try {
      ...
      // Получаем изображения из product_images и формируем прокси-ссылки
      const imgRes = await db.query(
      ...
      const siteBase = process.env.SITE_BASE_URL || 'https://moduletrade.ru';
      product.images = (imgRes.rows || []).map((row, idx) => {
        const url = row.image_url;
        if (!url) return row;
        const token = cryptoUtils.encrypt(JSON.stringify({ u: url, p: productId, i: row.id || idx }));
