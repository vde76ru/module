...
BEGIN
    ...
    WITH stats AS (
        SELECT
            ...
            jsonb_object_agg(action_type, action_count) as actions_by_type,
            jsonb_object_agg(user_name, user_action_count) as actions_by_user
        FROM (
            SELECT
                al.action_type,
                ...
                al.user_id,
                u.name as user_name,
                COUNT(*) OVER (PARTITION BY al.user_id) as user_action_count
            FROM audit_logs al
            LEFT JOIN users u ON al.user_id = u.id
            WHERE (p_company_id IS NULL OR al.company_id = p_company_id)
                AND (p_date_from IS NULL OR al.created_at >= p_date_from)
                AND (p_date_to IS NULL OR al.created_at <= p_date_to)
            GROUP BY al.action_type, al.user_id, u.name
        ) subq
    )
    SELECT
        COALESCE(stats.total_actions, 0),
        COALESCE(stats.users_count, 0),
        COALESCE(stats.tables_count, 0),
        COALESCE(stats.actions_by_type, '{}'::jsonb),
        COALESCE(stats.actions_by_user, '{}'::jsonb)
    FROM stats;
END;
$$ LANGUAGE plpgsql;
...
SELECT
    ...
    COALESCE(SUM(wpl.available_quantity), 0) as available_quantity,
    COALESCE(SUM(wpl.reserved_quantity), 0) as reserved_quantity,
    COALESCE(AVG(wpl.unit_cost), 0) as avg_price,
    COALESCE(MIN(wpl.unit_cost), 0) as min_price,
    COALESCE(MAX(wpl.unit_cost), 0) as max_price,
    MAX(wpl.last_updated) as last_updated
FROM products p
